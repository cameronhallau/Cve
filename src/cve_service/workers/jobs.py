...
class InlinePayloadProvider:
    def __init__(self, payload: dict[str, Any] | str, *, model_name: str = "inline-worker-provider") -> None:
        self.payload = payload
        self.model_name = model_name

    def review(self, request):  # pragma: no cover - exercised through worker integration tests
...
def process_post_enrichment_workflow_job(
    ...
    *,
    ...
    try:
        with session_scope(session_factory) as session:
            provider = InlinePayloadProvider(ai_payload or "{}", model_name=ai_model_name)
